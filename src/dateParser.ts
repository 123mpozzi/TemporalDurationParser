...
class ClassicParser {
  ...
  public static parse(
    str: string,
    keyMaps: Array<Array<DESIGNATORS>>,
    debug: boolean = false
    ...
    if (str.includes(DESIGNATORS.TIME)) {
      ...
      if (splits.length == 2) timeMap = ClassicParser.parseComponent(timeMap, splits[1], debug)
...
export class DateParser {
  ...
  private static parse(
    ...
    if (formatType === FORMAT_TYPE.CLASSIC) return ClassicParser.parse(str, keyMaps, debug)
    else return WeeksParser.parse(str, keyMaps, debug)
  }
